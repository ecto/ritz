...
console.log('handValue', handValue);
  ...
  if (!hasPlayedBefore) {
    // is it a pair?
    if (hand.cards[0].blackjackValue() == hand.cards[1].blackjackValue()) {
      return pairTable[hardCard][upcard];
    }

    // is it a soft hand?
console.log('isSoft', isSoft);
    if (isSoft) {
console.log(hardCard, upcard);
      return softTable[hardCard][upcard];
    }
  }

console.log('hard', handValue, upcard);
